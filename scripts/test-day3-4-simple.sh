...
echo "🧪 Testing Day 3-4: Rust API Implementation (Simplified)"
echo "======================================================"
...
echo ""
print_info "Step 4: Running database migrations using cargo..."

# Run migrations using cargo (since diesel CLI is included in the project)
if cargo run --bin diesel -- migration run; then
    print_success "Database migrations completed"
else
    print_warning "Migration failed, trying alternative approach..."
    # Try using the setup script if it exists
    if [ -f "setup.sh" ]; then
        chmod +x setup.sh
        if ./setup.sh; then
            print_success "Database setup completed via setup script"
        else
            print_error "Database setup failed"
            exit 1
        fi
    else
        print_error "No migration method available"
        exit 1
    fi
fi
...
echo ""
print_info "Step 7: Running API tests..."
